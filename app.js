...
function bindUI() {
  ...
  document.getElementById("recommendations")?.addEventListener("click", handleRecommendationGroupToggle);
...
function renderRecommendations(recommendations) {
  ...
  container.innerHTML = groupedRecommendations.map((group) => {
    ...
    const groupId = `recommendation-group-${escapeAttribute(group.page)}`;
    return `
      <section class="recommendation-group is-expanded" data-page-group="${escapeAttribute(group.page)}">
        <button class="recommendation-group-toggle" type="button" aria-expanded="true" aria-controls="${groupId}">
        ...
        <div id="${groupId}" class="recommendation-group-list">
          ${cards.join("")}
        </div>
      </section>
    `;
...
function handleRecommendationGroupToggle(event) {
  const toggle = event.target.closest(".recommendation-group-toggle");
  if (!toggle) return;

  const group = toggle.closest(".recommendation-group");
  const content = group?.querySelector(".recommendation-group-list");
