...
function renderReport(report) {
  ...
  const checksUsed = Number(report.checksUsed) || 0;
  const criticalIssues = Number(report.criticalIssues) || 0;
  const pagesAnalyzed = Number(report.pagesAnalyzed) || 0;
  const revenueOpportunity = report.revenueOpportunity || estimateRevenueOpportunity(overallScore, criticalIssues, pagesAnalyzed);

  const overallScoreNode = document.getElementById("overallScore");
  if (overallScoreNode) overallScoreNode.textContent = `${overallScore}/100`;

  const overallScoreLabelNode = document.getElementById("overallScoreLabel");
  if (overallScoreLabelNode) {
    overallScoreLabelNode.textContent = overallScore >= 80
      ? "0 to 100 scale · Strong CRO baseline"
      : overallScore >= 60
        ? "0 to 100 scale · Good start, room to improve"
        : "0 to 100 scale · Needs CRO attention";
  }

  const checksUsedNode = document.getElementById("checksUsed");
  if (checksUsedNode) checksUsedNode.textContent = String(checksUsed);

  const criticalIssuesNode = document.getElementById("criticalIssues");
  if (criticalIssuesNode) criticalIssuesNode.textContent = String(criticalIssues);

  const pagesAnalyzedNode = document.getElementById("pagesAnalyzed");
  if (pagesAnalyzedNode) pagesAnalyzedNode.textContent = String(pagesAnalyzed);

  const revenueOpportunityNode = document.getElementById("revenueOpportunity");
  if (revenueOpportunityNode) revenueOpportunityNode.textContent = `${revenueOpportunity}%`;

  const revenueOpportunityLabelNode = document.getElementById("revenueOpportunityLabel");
  if (revenueOpportunityLabelNode) {
    revenueOpportunityLabelNode.textContent = "Heuristic upside from fixing the current gaps";
  }

  safeRenderSection("home speed metric", () => renderHomeSpeedMetric(report.homePageSpeed));

  const gauge = document.getElementById("scoreGaugeFill");
  if (gauge) gauge.style.width = `${Math.max(0, Math.min(100, overallScore))}%`;

  safeRenderSection("inspection quality", () => renderInspectionQuality(report));
  safeRenderSection("recommendations", () => renderRecommendations(Array.isArray(report.recommendations) ? report.recommendations : []));
  safeRenderSection("required elements", () => renderRequiredElementsResults(report.requiredElementsAudit));
  safeRenderSection("page breakdown", () => renderPageBreakdown(Array.isArray(report.pageResults) ? report.pageResults : []));
  safeRenderSection("stack insights", () => renderStackInsights(report.stackSummary));
  safeRenderSection("competitor comparison", () => renderCompetitorComparison(report.competitorReport));
  safeRenderSection("screenshots", () => renderScreenshots(Array.isArray(report.pageResults) ? report.pageResults : []));
  safeRenderSection("discovered URLs", () => renderDiscoveredUrls(report.discovered || STATE.discovered));
}
